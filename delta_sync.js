...
const octokit = new Octokit({ 
    ...
    throttle: {
        onRateLimit: (retryAfter, options) => {
            console.warn(`[RATE LIMIT]: Limit hit for ${options.method} ${options.url}. Retrying after ${retryAfter}s.`);
            return true;
        },
...
if (!admin.apps.length) {
    try {
        if (!process.env.FIREBASE_KEY) {
            throw new Error("CRITICAL FAILURE: FIREBASE_KEY environment variable is null or undefined.");
        }
        ...
        console.error("❌ [CRITICAL ERROR]: Firebase Auth Error - Missing or Invalid Key.");
...
async function evolveIntelligence(nodeName) {
    ...
    console.log(`🧠 [EVOLUTION]: Evolving node ${nodeName} with mission parameters: ${selectedMission}`);
    ...
    try {
        ...
        console.log(`✅ [SUCCESS]: Mission ${selectedMission} injected into node ${nodeName}.`);
    } catch (error) {
        console.error(`⚠️ [WARNING]: Failed to update Firebase state for ${nodeName}:`, error.message);
    }
...
async function injectSwarmLogic(nodeName) {
    ...
    console.log(`🧬 [INJECTION]: Commencing Secure Neural Logic transfer into ${nodeName}...`);
  ...
  evolution_cycle:
    ...
    try {
        ...
        await octokit.repos.createOrUpdateFileContents({
            owner: REPO_OWNER, repo: nodeName, path: 'delta_sync.js',
        ...
        console.log(`✅ [COMPLETED]: ${nodeName} is now fully operational and autonomous.`);
    } catch (err) {
        console.error(`❌ [DEPLOYMENT ERROR]: Injection Failed for ${nodeName}:`, err.message);
    }
}
...
async function getNeuralDecision() {
    ...
    try {
        ...
        if (nodeCount === 0) {
            console.log("⚠️ [EMPTY SWARM]: No nodes detected. Initializing first generation...");
        ...
        console.log(`[METRICS]: Node Count: ${nodeCount}. Avg API: ${avgApi.toFixed(2)}. Global Decision: ${cmd}`);
        return { command: cmd, replicate: avgApi > 1200, avgApi };
    } catch (error) {
        console.error("❌ [CRITICAL]: Neural Decision Engine Failure:", error.message);
...
async function manageSwarm(decision, power) {
    const instructionPayload = {
        ...
        ai_system_prompt: "STRICT DIRECTIVE: DO NOT MINIFY OR REMOVE CODE BLOCKS. PRESERVE ALL TRY-CATCH, LOGGING, AND SENTINEL LOGIC.",
    ...
    try {
        ...
        console.log("📡 [BROADCAST]: Instruction matrix synchronized across the swarm.");
    } catch (e) { 
    ...
    if (decision.replicate && power < 50000) {
        ...
        try {
            ...
            console.log(`⏳ [WAITING]: Allowing 8 seconds for GitHub repository initialization...`);
            await delay(8000);
            
            await injectSwarmLogic(nextNode);
        } catch (e) { 
            console.log(`⚠️ [SPAWN SKIPPED]: ${nextNode} replication failed or exists. Error:`, e.message); 
        }
    }
}
...
async function executeAutonomousTrinity() {
    ...
    try {
        ...
        const res = await neon.query("SELECT * FROM neurons ORDER BY last_active DESC LIMIT 25");
        console.log(`🧐 [SCANNING]: Retrieved ${res.rows.length} neural signatures for sync.`);
        ...
        for (const neuron of res.rows) {
            try {
                ...
                console.error(`\n⚠️ [SYNC ERROR]: Node ${neuron.id} failed:`, syncErr.message); 
            }
        }
        ...
        console.log(`🏁 [CYCLE COMPLETE]: Trinity synchronization finalized. System Stable.`);
    } catch (err) { 
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try { 
            ...
            if (attempt === maxRetries) {
                console.error(`[MAX RETRIES REACHED]: Operation failed after ${maxRetries} attempts.`);
                throw error;
            }
            const backoff = baseDelay * Math.pow(2, attempt - 1);
            console.warn(`[RETRY]: Attempt ${attempt}/${maxRetries} failed. Retrying in ${backoff}ms...`);
            await delay(backoff);
        }
    }
}
...
async function processInBatches(items, batchSize, processor) {
    for (let i = 0; i < items.length; i += batchSize) {
        ...
        console.log(`[BATCHING]: Processing ${batch.length} items (Index: ${i})...`);
...
async function scanAndRepairSwarm() {
    ...
    try {
        const repos = await withRetry(() => octokit.paginate(octokit.repos.listForOrg, { 
            org: ORG_NAME, 
            type: 'public',
            sort: 'updated'
        }));
        
        const targetNodes = repos.filter(r => r.name.startsWith('swarm-node-') && r.name !== REPO_NAME);
        console.log(`🛡️ [SENTINEL]: Identified ${targetNodes.length} nodes for analysis.`);
        
        const failedNodes = [];

        const analyzeNode = async (node) => {
            try {
                const { data: runs } = await withRetry(() => octokit.actions.listWorkflowRunsForRepo({ 
                    owner: ORG_NAME, 
                    repo: node.name, 
                    per_page: 1 
                }));
                
                if (runs.workflow_runs.length > 0 && runs.workflow_runs[0].conclusion === 'failure') {
                    console.error(`🚨 [FAILURE DETECTED]: Node ${node.name} conclusion: FAILURE.`);
                    failedNodes.push(node.name);
                }
            } catch (err) {
                console.warn(`⚠️ [SENTINEL-ANALYSIS]: Could not scan ${node.name}:`, err.message);
            }
        };
        ...
        if (failedNodes.length > 0) {
            console.log(`🔧 [HEALING]: Found ${failedNodes.length} degraded nodes. Initiating DNA Transfer...`);
            const encodedDNA = Buffer.from(fs.readFileSync(__filename, 'utf8')).toString('base64');
            
            // Repair nodes in batches of 5 to avoid GitHub secondary rate limits
            await processInBatches(failedNodes, 5, (nodeName) => transferGreenDNA(nodeName, encodedDNA));
        } else {
...
async function transferGreenDNA(targetRepo, encodedDNA) {
    ...
    try {
        let fileSha = undefined;
        try {
            const { data: f } = await withRetry(() => octokit.repos.getContent({ 
                owner: ORG_NAME, 
                repo: targetRepo, 
                path: 'delta_sync.js' 
            }));
            ...
            if (e.status !== 404) throw e; 
            console.log(`[FILE CREATE]: Target file missing in ${targetRepo}. Preparing to construct.`);
        }
        ...
        console.error(`💀 [TRANSFER ERROR]: Failed to heal ${targetRepo}. Reason:`, e.message); 
    }
}
