...
const MANUS_TOOLS = [
    {
        ...
        function: {
            ...
            parameters: {
                type: "object",
                properties: {
                    url: { type: "string", description: "ဝင်ရောက်ကြည့်ရှုမည့် URL လမ်းကြောင်း" }
                },
                required: ["url"]
            }
        }
    },
    {
        ...
        function: {
            ...
            parameters: {
                type: "object",
                properties: {
                    command: { type: "string", description: "မောင်းနှင်မည့် Terminal Command" }
                },
                required: ["command"]
            }
        }
...
/**
 * 🌌 MANUS-STYLE AUTONOMOUS AGENT EXECUTION ENGINE
 * ၎င်းသည် Node အား လွတ်လပ်စွာ တွေးတောပြီး Tools များ အသုံးပြုကာ အလုပ်ပြီးအောင် လုပ်ခွင့်ပေးသည်။
 */
async function executeAgenticGhost(target, mainInstruction) {
    ...
    // AI အတွက် ပေးမယ့် အလုပ်
    let taskPrompt = `မင်းရဲ့ လက်ရှိပန်းတိုင်ကတော့: Target Node ${target.name} (NORAD: ${target.norad_id}) ကို စစ်ဆေးဖို့ဖြစ်တယ်။ 
    ပင်မညွှန်ကြားချက်: ${mainInstruction}.
    မင်းမှာရှိတဲ့ Tools တွေကို အသုံးပြုပြီး လိုအပ်တဲ့ ဒေတာတွေကို ရှာဖွေပြီး အလုပ်ပြီးဆုံးအောင် လုပ်ဆောင်ပါ။`;

    let messages = [
        { 
            role: "system", 
            content: "You are an autonomous Manus-style AI agent acting as a Swarm Node. You must accomplish the task by calling available tools step-by-step. Analyze the tool outputs and decide the next move. When fully done, provide a final verification report." 
        },
        { role: "user", content: taskPrompt }
    ];

    const MAX_STEPS = 5; // AI လမ်းပျောက်ပြီး Loop အပတ်လွန်မသွားအောင် ထိန်းထားခြင်း
    let currentStep = 0;
    let keepRunning = true;

    while (keepRunning && currentStep < MAX_STEPS) {
        currentStep++;
        console.log(`🧠 [AGENT-THOUGHT]: Node Processing Step ${currentStep}/${MAX_STEPS}...`);

        try {
            // Groq Core API ကို Tools တွေပါ တွဲပေးပြီး လှမ်းခေါ်ခြင်း
            const response = await axios.post("https://api.groq.com/openai/v1/chat/completions", {
                model: "llama-3.3-70b-versatile",
                messages: messages,
                tools: MANUS_TOOLS,
            ...
            messages.push(message); // Conversation History ထဲ ထည့်မယ်

            // AI က Tools သုံးဖို့ ဆုံးဖြတ်လိုက်ရင် (Manus Stage)
            if (message.tool_calls && message.tool_calls.length > 0) {
                for (const toolCall of message.tool_calls) {
                    const toolName = toolCall.function.name;
                    const toolArgs = JSON.parse(toolCall.function.arguments);
                    console.log(`🛠️ [MANUS-ACTION]: Agent triggered execution for tool: '${toolName}'`);

                    let toolResult = "";

                    // လက်တွေ့ Tool တွေကို Node.js ထဲမှာ ပတ်မောင်းပြီး AI ဆီ ရလဒ် ပြန်ကျွေးခြင်း
                    if (toolName === "fetchWebContent") {
                        try {
                            // မင်းရဲ့ axios သုံးပြီး URL ထဲက ဒေတာ သွားဆွဲတဲ့အပိုင်း
                            const webRes = await axios.get(toolArgs.url, { timeout: 5000 });
                            toolResult = JSON.stringify(webRes.data).substring(0, 1500); // စာလုံးအရှည်ကြီးဖြစ်မသွားအောင် ဖြတ်ထုတ်ခြင်း
                        ...
                        try {
                            // child_process.execSync သုံးပြီး code တွေကို local မှာ run စေခြင်း
                            const stdout = execSync(toolArgs.command, { encoding: 'utf8', timeout: 5000 });
                            toolResult = stdout;
                            console.log(`✅ [TOOL-SUCCESS]: Command executed successfully.`);
                        } catch (err) {
                            toolResult = `Command Execution Failed: ${err.message}`;
                        }
                    }

                    // Tool က ထွက်လာတဲ့ ရလဒ်ကို Messages Array ထဲ ပြန်ထည့်ပြီး AI ကို နောက်တစ်ဆင့် ဆက်စဉ်းစားခိုင်းခြင်း
                    messages.push({
                        role: "tool",
                ...
                // AI က Tools ထပ်မသုံးတော့ဘဲ အဖြေထွက်လာပြီဆိုရင် Loop ကို ရပ်လိုက်မယ်
                console.log(`🏁 [AGENT-FINAL-REPORT]:\n${message.content}`);
                keepRunning = false;
                return {
                    node: target.norad_id,
                    status: "COMPLETED",
                    agent_report: message.content
                };
            }
...
// Replace near the bottom of executeHyperOrbitalSovereign in cluster_sync.js

// Old Code:
// await Promise.all(plans.map(target => executeHyperGhost(target)));

// New Code: Execute Manus-style autonomous agent loop for targets
for (const target of plans) {
    const mainInstruction = "Analyze the security architecture of modern technology environments.";
...
async function consultSovereignAI() {
    ...
    // 🛡️ externalLogic မရှိရငျ အောကျက replace တှေ ဆကျမလုပျဘဲ လုံခွုံစှာ ထှကျသှားရနျ ကာကှယျခွငျး
    if (!externalLogic) {
        console.log("⚠️ [WARNING]: </SOVEREIGN_CORE> tag missing or external logic is empty.");
        return null;
    }
...
await new Promise(resolve => setTimeout(resolve, 4000)); // ၄ စကျကန့နြားမညြ
...
// === 🔱 LEVEL 4 INNOVATOR MODULE ===
async function analyzeAndOptimizeSelf() {
    // API Quota မကုနျအောငျ ၅ ကွိမျလညျမှ ၁ ကွိမျသာ အလုပျလုပျရနျ
    if (timeT % 5 !== 0) return null; 
    
    console.log("🧬 [LEVEL 4 INNOVATOR]: Scanning for logic optimization...");
    try {
        const KEY = process.env.GROQ_API_KEY;
        const response = await axios.post("https://api.groq.com/openai/v1/chat/completions", {
            model: "llama-3.1-8b-instant",
            messages: [{ 
                role: "system", 
                content: "Suggest one short optimization strategy for a Node.js autonomous agent. Return only a short, concise sentence." 
            }],
            temperature: 0.7
        ...
        return "Optimization paused. Preserving API limits.";
    }
}
// ===================================
...
// --- LEVEL 4 TRIGGER ---
        const innovationFeedback = await analyzeAndOptimizeSelf();
        if (innovationFeedback) {
            console.log(`💡 [NEW INNOVATION]: ${innovationFeedback}`);
            globalHomeostasis += 1.0; // AI ရဲ့ Homeostasis ကို တကစြရနြေ
        }
        // -----------------------    
    ...
    try {
        ...
        const injectToResearch = "INSERT INTO research_data (title, detail, harvested_at) VALUES ($1, $2, NOW()) ON CONFLICT (title) DO NOTHING;";
...
async function startGodMode() {
    try {
        ...
        process.exit(0); // ✅ အလုပပြှီးရငြ ပိတလြိုကပြါ
    } catch (err) {
        console.error("⚠️ [GOD-MODE] Protocol Breach detected!", err);
        process.exit(1); // ❌ Error တကရြငြ အတငြးရပပြဈပါ
    }
}
bootSystem();
// </SOVEREIGN_CORE>
